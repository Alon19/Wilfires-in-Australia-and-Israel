---
title: "Predicting Fires by Temperature and Rainfall"
...
```{r import_data}
aus_fires <- read.csv("./data/temperature-rainfall-model/filtered_aus_fires.csv")
isr_fires <- read.csv("./data/temperature-rainfall-model/isr_fires_w_confidence.csv")
...
rec <- recipe(
  confidence_level ~ temperature + rainfall_7d_sum + distance_from_rain_station + distance_from_temp_station, data=train_aus_fire 
  )
...
aus_pred_df %>%
  ...
  labs(
    title = "ROC Curve",
    subtitle= "Confidence Level by Temperature and Rainfall amount",
    x = "FP Rate",
    y = "TP Rate")
```
...
aus_pred_df %>% group_by(confidence_level,pred_1) %>%
  ...
  labs(
    title = "Confidence Level by Temperature and Rainfall Amount",
...
#### 3.1. Preview the test data
```{r preview_isr, warning=F, message=F}
kable(head(isr_fires %>% select(columns_to_show)), align="c")
```

#### 3.2. Predict the confidence level of each record in the Test data
```{r predict_isr}
isr_pred_df <- predict(conf_fit, isr_fires, type = "prob") %>% 
  bind_cols(isr_fires)
```
...
isr_pred_df %>%
  ...
  labs(
    title = "ROC Curve",
    subtitle= "Confidence Level by Brightness and FRP",
    x = "FP Rate",
    y = "TP Rate")
```
...
isr_pred_df %>% group_by(confidence_level,pred_1) %>%
  summarise(Freq = n()) %>% as.data.frame() %>% 
  ggplot(aes(x=confidence_level, y=pred_1, fill=Freq)) + geom_tile() + 
  geom_text(aes(label = Freq), size = 6) +
  scale_fill_gradient(low = "white", high = "#3575b5") + 
